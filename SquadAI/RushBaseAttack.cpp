...
void RushBaseAttack::Initiate(void)
{
	...
	for(std::vector<Entity*>::iterator it = m_participants.begin(); it != m_participants.end(); ++it)
	{
		Order* pNewOrder = new MoveOrder((*it)->GetId(), MoveToPositionOrder, MediumPriority, XMFLOAT2(m_assemblyPoint));
...
float RushBaseAttack::GetAssemblyPointDistance(void) const
{
	return m_assemblyPointDistance;
}
...
void RushBaseAttack::SetAssemblyPointDistance(float distance)
{
	m_assemblyPointDistance = distance;
}